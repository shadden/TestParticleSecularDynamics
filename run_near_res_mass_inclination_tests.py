...
sim = rb.Simulation()
sim.add(m=1)
add_canonical_heliocentric_elements_particle(m_pl0,{'a':1,'inc':0.01,'l':0,'pomega':0},sim)
sim.N_active = sim.N

a_tp = (period_ratio)**(2/3)
tp_h = TestParticleSecularHamiltonian(a_tp,secular_theory)

for i_free in np.linspace(0.001,0.02,5):
    y_tot  = 2*np.sin(0.5*i_free) * np.exp(1j * 0.5 * np.pi) + np.sum(list(tp_h.F_inc.values()))
    i_tot = 2*np.arcsin(np.abs(y_tot)/2)
    add_canonical_heliocentric_elements_particle(0,{'a':a_tp,'e':0,'inc':i_tot,'l':np.pi,'Omega':np.angle(y_tot)},sim)
...
mass_growth_file = f"./archives/inc_mass_growth_j_{jres:d}_k_{kres:d}_Delta_{Delta:.4f}.sa"
...
for i,mass in enumerate(np.linspace(1e-6,0.45e-5,10)):
    ...
    sim_new.save_to_file(f'./archives/inc_test_particles_mass_{i}_j_{jres:d}_k_{kres:d}_Delta{Delta:.4f}.sa',step = steps,delete_file=True)
